...
export async function getSafeImage(src: Buffer) {
    ...
    try {
        ...
        const unitSize = 16,
            unitCount = 8,
            channels = 4;
        safeImage = Buffer.alloc(unitSize * unitSize * unitCount * unitCount * channels);
 ...
 */
export async function safeLoadImage(src: Buffer) {
    return loadImage(await getSafeImage(src));
}
